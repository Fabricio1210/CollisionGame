...
public class Shooter extends MovingObject{
	...
	public Shooter(Vector2D position, Vector2D velocity, double maxVel, BufferedImage texture,
		...
		fireRate.run(Constants.SHOOTER_FIRE_RATE);
	}
	...
	public void update() {
		...
		pathFollowing = pathFollowing.scale(1/Constants.SHOOTER_MASS);
		...
		if(!fireRate.isRunning()) {
			...
			currentAngle += Math.random()*Constants.SHOOTER_ANGLE_RANGE - Constants.SHOOTER_ANGLE_RANGE / 2;
			...
			EnemyBullets bullet = new EnemyBullets(
			...
			gameState.getMovingObjects().add(0, bullet); 
	...
	@Override
	public void destroy(){
		gameState.addScore(Constants.SHOOTER_SCORE, position);
		super.destroy();
	}
